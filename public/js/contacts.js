...
function renderContactsList() {
  ...
  if (filteredGroups.length) {
    ...
    if (!grpCollapsed) {
      h += filteredGroups.map(g => { const sgid = escJs(g.id); const isCustom = customGroups.some(x => x.id === g.id); const settingsBtn = canManage() && isCustom ? `<button class="contact-train-btn" onclick="event.stopPropagation();openGroupSettings('${g.id}')" title="${t('contacts.groupSettings') || '群组设置'}">&#9881;</button>` : ''; return `<div class="contact-item" onclick="startChat('${sgid}')"><div class="contact-avatar">${g.emoji}</div><div class="contact-info"><div class="contact-name">${esc(g.name)}</div><div class="contact-desc">${esc(g.desc)}</div></div><div class="contact-action">${settingsBtn}<button onclick="event.stopPropagation();startChat('${sgid}')">${t('contacts.startChat')}</button>${canManage() && isCustom ? `<button class="btn-icon-danger" onclick="event.stopPropagation();deleteGroupConfirm('${sgid}')" title="${t('panel.dissolve')}">&#128465;</button>` : ''}</div></div>`; }).join('');
