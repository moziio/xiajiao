...
::: warning better-sqlite3 编译问题
极少数情况下 `npm install` 会提示 `better-sqlite3` 编译失败。解决方案：
...
大多数情况下不需要这一步，因为 `better-sqlite3` 会自动下载预编译的二进制文件。
:::
...
**症状**：`gyp ERR!` 或 `node-pre-gyp` 相关错误

**原因**：`better-sqlite3` 或 `sharp` 的原生模块编译失败
