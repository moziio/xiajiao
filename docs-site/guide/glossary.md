...
→ [多 Agent 群聊](/features/multi-agent-chat)
...
→ [Agent 持久记忆](/features/agent-memory)

## F

### FTS5

SQLite 的全文搜索引擎（Full-Text Search 5）。虾饺用 FTS5 做消息历史搜索和 RAG 的 BM25 关键词检索。
...
→ [SOUL.md 写作指南](/guide/soul-guide) · [SOUL.md 模板库](/guide/soul-templates)
...
→ [性能调优](/guide/performance)

### WebSocket

全双工通信协议。虾饺用 WebSocket（`ws` 库）实现消息的实时推送和流式输出。

### Workspace

Agent 的工作空间目录（`data/workspace-{id}/`）。包含 Agent 的 SOUL.md、记忆数据库、RAG 知识库文件。
...
→ [协作流](/features/collaboration-flow)

### 虾饺

广式点心。薄皮半透明，内包鲜虾。项目取名自此——小巧精致，内料丰富。
