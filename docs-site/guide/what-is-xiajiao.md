...
| 包 | 作用 | 为什么不能去掉 | 替代方案 |
|---|------|--------------|---------|
| `ws` | WebSocket | Node.js 标准库没有 WebSocket 服务端 | 无合适替代 |
| `better-sqlite3` | 同步 SQLite | `node:sqlite` 是异步的，embedding 计算需要同步 API | 无合适替代 |
| `marked` | Markdown 渲染 | SOUL.md 和消息中的 Markdown 需要转 HTML | 可以不渲染，但体验差 |
| `pdf-parse` | PDF 解析 | RAG 知识库需要读取 PDF 文本 | 不支持 PDF 上传 |
| `highlight.js` | 代码高亮 | 代码块着色，开发者核心体验 | 纯文本显示 |
| `sharp` | 图片处理 | 上传图片生成缩略图 | 不压缩直接存 |
