...
虾饺内置生产级 RAG（Retrieval-Augmented Generation）系统。上传文档，Agent 自动索引，提问时通过 [Tool Calling](/zh/features/tool-calling) 中的 `rag_query` 检索——你的文档变成 Agent 的"大脑"。与 [Agent 持久记忆](/zh/features/agent-memory) 搭配：RAG 管文档事实，记忆管用户偏好与历史。
...
Agent 通过 `rag_query` 工具（见 [Tool Calling](/zh/features/tool-calling)）自动检索：
...
- [Tool Calling](/zh/features/tool-calling) — `rag_query` 工具详情与权限配置
- [Agent 持久记忆](/zh/features/agent-memory) — 个性化记忆系统
- [安全与隐私](/zh/guide/security) — 数据安全与 API Key 保护
- [协作流](/zh/features/collaboration-flow) — 多 Agent 自动协作
- [实战案例](/zh/guide/recipes) — 私人知识助理、客户支持等 RAG 实战方案
- [架构设计](/zh/guide/architecture) — RAG 管线的代码实现详解
