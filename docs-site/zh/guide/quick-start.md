...
| `pdf-parse` | PDF 文本提取 | [RAG 知识库](/zh/features/rag)需要从 PDF 提取文字 |
...
各厂商的 Base URL、模型名与排错见 [模型配置大全](/zh/guide/model-config)。
...
虾饺本身只提供 HTTP。生产环境建议用 Nginx 反向代理加 HTTPS。详见 [云服务器部署](/zh/deployment/cloud)。
...
8. 在通讯录中创建一个新 Agent
9. 编辑它的 [SOUL.md](/zh/guide/soul-guide)，写入你需要的人格（参考 [模板库](/zh/guide/soul-templates)）
10. 给它配置合适的工具和模型
...
12. 上传文档到 RAG 知识库
13. 设置定时任务（每天新闻摘要）
14. 尝试 [实战案例](/zh/guide/recipes) 中的配置方案

## 下一步

| 你想... | 看这里 |
|---------|--------|
| 详细安装步骤 | [安装指南](/zh/guide/installation) — Windows / macOS / Linux |
| 体验群聊 | [多 Agent 群聊](/zh/features/multi-agent-chat) — 创建群组，@mention |
| 配更多模型 | [模型配置](/zh/guide/model-config) — 8 个 Provider 详细教程 |
| 用 Docker | [Docker 部署](/zh/deployment/docker) — 容器化部署 |
| 写 Agent 人格 | [SOUL.md 指南](/zh/guide/soul-guide) — 写出好的人格设定 |
| 照搬方案 | [实战案例](/zh/guide/recipes) — 12 个 Agent 团队配置 |
| 遇到问题 | [故障排查](/zh/guide/troubleshooting) — 按症状排查 |

## 相关文档

- [模型配置大全](/zh/guide/model-config) — 各 Provider 的 API 与模型名、排错清单
- [SOUL.md 编写指南](/zh/guide/soul-guide) — 定义 Agent 角色与专长
- [实战案例](/zh/guide/recipes) — 12 个可复制的 Agent 团队配置
