...
详见 [协作流](/zh/features/collaboration-flow)。

### 消息历史会保存吗？

是的，所有消息永久保存在 SQLite 数据库中（`data/xiajiao.db`）。支持全文搜索（FTS5）。
...
支持。虾饺作为 **MCP 客户端**：在 **设置 → MCP** 中连接外部 MCP 服务（stdio 或 HTTP）；工具会通过 JSON-RPC 能力协商自动注册为 `mcp:{serverId}:{toolName}`，供 Agent 使用。

### 能二次开发吗？

当然。MIT 协议，目录清晰，可从 `server/routes`、`server/services`、`public/` 等入手扩展。详见 GitHub `CONTRIBUTING.md` 与 [开发者指南](/zh/guide/dev-guide)。
...
详见 [模型配置大全](/zh/guide/model-config)。
...
详见 [安全与隐私](/zh/guide/security)。

## 进阶开发

### 怎么给虾饺添加一个新的 API 端点？

在 `server/routes/` 下编辑相应模块，并在 `server/router.js` 中注册路由。重启 Node 后生效。

### 怎么添加自定义工具？

共有三种方式：

1. **HTTP 自定义工具（零代码）**：**设置 → HTTP 工具** — 将任意 REST API 配成 Agent 工具，支持 `{{param}}` 插值，无需写代码  
2. **JS 自动注册**：将 `.js` 文件放入 `server/services/tools/` 或 `data/custom-tools/` — 启动时自动注册  
3. **MCP 桥接**：在 **设置 → MCP** 中连接外部 MCP 服务  

详见 [Tool Calling — 自定义工具](/zh/features/tool-calling#自定义工具) 与 [开发者指南](/zh/guide/dev-guide)。

### 虾饺的数据库 Schema 是什么？

详见 [API 与协议参考](/zh/guide/api-reference#数据库表结构)——包含所有表结构、字段说明。
...
- [快速开始](/zh/guide/quick-start) — 3 步跑起来
- [模型配置](/zh/guide/model-config) — 详细的模型配置教程
- [多 Agent 群聊](/zh/features/multi-agent-chat) — 核心功能介绍
- [故障排查](/zh/guide/troubleshooting) — 遇到问题看这里
- [安全与隐私](/zh/guide/security) — 数据安全详解
- [平台对比](/zh/guide/comparison) — 和 Dify/Coze/FastGPT 的区别
- [API 参考](/zh/guide/api-reference) — 接口和协议详情
- [术语表](/zh/guide/glossary) — 不懂的术语看这里
